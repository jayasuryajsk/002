...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!documentsLoaded || refresh) {
      // First try to fetch from sources API
      console.log('Fetching source documents from sources API...');
      ...
      try {
        ...
        if (sourcesResponse.ok) {
          ...
          if (Array.isArray(sourcesData) && sourcesData.length > 0) {
            console.log(`Found ${sourcesData.length} source documents from API call`);
            sourceDocuments = sourcesData;
            documentsLoaded = true;
          } else {
            console.warn('Sources API returned empty array or invalid data');
          }
        } else {
          console.error(`Sources API returned error: ${sourcesResponse.status}`);
        }
      } catch (apiError) {
        console.error('Error fetching from sources API:', apiError);
      }
      
      // If we still don't have source documents, try loading from storage
      if (sourceDocuments.length === 0) {
    ...
    return NextResponse.json(
      { error: 'Failed to process indexing request', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
} 