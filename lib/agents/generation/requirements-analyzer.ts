...
export async function extractRequirementsFromTender(tenderText: string): Promise<TenderRequirement[]> {
  console.log("Extracting requirements from tender document...")
  
  const prompt = `
Extract all requirements from the following tender document. 
Identify each distinct requirement, categorize it, and assign a priority level.
...
2. For each requirement provide:
   ...
   - The full description as stated in the document
   - A priority level (high, medium, or low) based on emphasis and importance in the document
   - A category (e.g., "Technical", "Experience", "Compliance", "Financial", etc.)
3. Return the results as a JSON array with the following structure:
[
  {
    "id": "REQ-1",
...
Assign IDs sequentially as REQ-1, REQ-2, etc.
`;
  ...
  try {
    ...
    if (!jsonMatch) {
      console.log("No valid JSON found in response:", result.text.substring(0, 200) + "...");
      return [];
    }
    
    try {
      const requirementsJson = JSON.parse(jsonMatch[0]);
