...
export class PineconeSearchAgent {
   ...
   */
  async generateTenderSection(sectionTitle: string, relevantDocuments: SearchResult[], requirements: string[] = []): Promise<string> {
    try {
      ...
      const sourceDocsMap = new Map(sourceDocs.map(doc => [doc.id, doc]));
...
Write a comprehensive tender section titled "${sectionTitle}" based on the provided documents and requirements.
...
`;
      ...
      // Format requirements text if any
      const requirementsText = requirements.length > 0 
        ? `Requirements for this section:\n${requirements.map((req, i) => `${i+1}. ${req}`).join('\n')}`
        : 'No specific requirements provided for this section.';
      
      // First add relevant documents - preferring binary data over text
      let documentCount = 0;
      for (const doc of relevantDocuments) {
        // Try to find the original document with binary data
        const originalDoc = sourceDocsMap.get(doc.id);
        
        if (originalDoc && originalDoc.binaryData) {
          // If we have binary data (like PDF), add as inline data
          parts.push({
            inlineData: {
              data: Buffer.from(originalDoc.binaryData).toString("base64"),
              mimeType: originalDoc.metadata?.fileType || "application/pdf"
            }
          });
          documentCount++;
          console.log(`Added binary document: ${originalDoc.title}`);
        } else {
        ...
        // Limit to max 3 documents per section to avoid token limits
        if (documentCount >= 3) {
          console.log(`Limiting to ${documentCount} documents for section: ${sectionTitle}`);
          break;
        }
      }
      
      // Add requirements text
      parts.push({ text: requirementsText });
      
      // Add the instruction text at the end
      parts.push({ text: instructionText });
      
      // Generate the tender section using Gemini
      console.log(`Generating tender section: ${sectionTitle} with ${parts.length} parts`);
      const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash-001" });
      const result = await model.generateContent(parts);
      return result.response.text();
    } catch (error) {
      console.error("Section generation error:", error);
      return `# ${sectionTitle}\n\nUnable to generate this section due to an error: ${error instanceof Error ? error.message : String(error)}. Please try again later.`;
    }
  }
