...
export class TenderOrchestrator extends BaseAgent {
  ...
  constructor() {
    super(
      "orchestrator",
      "You are the coordinator of a multi-agent system for tender generation. Your role is to break down tasks, assign them to specialized agents, and monitor quality.",
  ...
  private async retrieveRelevantDocuments(
    ...
    try {
      ...
      console.log(`Retrieved ${sourceDocs.length} documents from storage for section: ${sectionTitle}`);
      ...
      return sourceDocs.slice(0, 3).map(doc => ({
        id: doc.id,
        content: doc.content || "Binary document",
        score: 1.0,
        metadata: doc.metadata || {},
